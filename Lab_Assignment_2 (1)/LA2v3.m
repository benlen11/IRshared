...
transSpoU3{1} = transl(-1.5, -1, 0.55);
transSpoU3{2} = transl(0.5, -1, 0.55);
transSpoU3{3} = transl(0.5, -0.9, 0.55);
transSpoU3{4} = transl(-1.5, -0.9, 0.55);

% setup RMRC translate pass 2
transSpoU3{5} = transl(-1.5, 1.5, 0.55);
transSpoU3{6} = transl(0.5, 1.3, 0.55);
transSpoU3{7} = transl(0.5, 1.5, 0.55);
...
for j = 1:1:4-1
    ...
    qMatrixU3(1,:) = rl.model.ikcon(transSpoU3{1}, moveSpoUIG{1});
    % qMatrixU3(1, :) = qU1;

    disp(rl.model.fkine(rl.model.getpos));
